...
object Settings {
  ...
  lazy val desktop = core ++ Seq(
    libraryDependencies ++= Seq(
      "net.sf.proguard" % "proguard-base" % "5.2" % "provided",
  ...
  lazy val android = core ++ Tasks.natives ++ androidBuild ++ Seq(
    ...
    apkbuildExcludes in Android ++= Seq("META-INF/LICENSE","META-INF/LICENSE.txt", "META-INF/NOTICE",  "META-INF/NOTICE.txt"),
...
object LibgdxBuild extends Build {
  ...
  lazy val all = Project(
    ...
    base     = file("."),
    settings = Settings.core
  ).aggregate(common, core, desktop, android, server)
